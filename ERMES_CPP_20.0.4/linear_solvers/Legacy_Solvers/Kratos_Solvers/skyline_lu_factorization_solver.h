...
namespace Kratos
{
    ...
    class LUSkylineFactorization 
    {
	 public:
        ...
        //*
        //***************************************************************************************************
        void copyFromCSRMatrix(CSRMatrix<double>& A) 
        {
            ...
            else 
            {
                ...
                for (i=0; i<size; i++) 
                {
                    degree[i]      = CSRSpace<double>::GraphDegree(i, A);
                ...
                while (next<size) 
                {
                    ...
                    for (soughtDegree=firstVal; soughtDegree!=finalVal; soughtDegree+=increment) 
                    {
                        ...
                        {
	                        // visit neighbors
	                        CSRSpace<double>::GraphNeighbors(node, A, neighbors);
        ...
        //*
        //***************************************************************************************************
        void backForwardSolve(int vector_size, Vector<double>& b, Vector<double>& x) // n, b, x
        {
	        // y = L^-1 * perm[b] ;
	        // y = U^-1 * y ;
	        // x = invperm[y];
    ...
    class SkylineLUFactorizationSolver : public DirectSolver<TSparseSpaceType, TDenseSpaceType, TReordererType>
    {
     public:
        ...
        */
        bool Solve(SparseMatrixType& rA, VectorType& rX, VectorType& rB)
        {
            ...
            LUSkylineFactorization myFactorization;
