...
#if !defined(KRATOS_ILUN_PRECONDITIONER_H_INCLUDED )
#define  KRATOS_ILUN_PRECONDITIONER_H_INCLUDED
...
namespace Kratos
{
    ...
    class ILUNPreconditioner : public ILUPreconditioner<TSparseSpaceType, TDenseSpaceType>
    {
    public:
     
      /// Counted pointer of ILUNPreconditioner
      typedef boost::shared_ptr<ILUNPreconditioner> Pointer;
      ...
      /// Default constructor.
      ILUNPreconditioner(int NewLevelNeighbors = 0) : levelNeighbors(NewLevelNeighbors) {}

      /// Copy constructor.
      ILUNPreconditioner(const ILUNPreconditioner& Other) : levelNeighbors(Other.levelNeighbors){}

      /// Destructor.
      virtual ~ILUNPreconditioner()
      {	
      }
      
    
      /// Assignment operator.
      ILUNPreconditioner& operator=(const ILUNPreconditioner& Other)
	{
	...
	{
	// ILUN(levelNeighbors) preconditioner
	// Santiago Egido, 2004
  ...
  for (i=0; i<n; i++) if (U[iU[i]]==0.00) {KRATOS_ERROR(std::runtime_error, "ILUNPreconditioner::Initialize", "Zero in U diagonal found!!", "")}
  ...
  inline std::istream& operator >> (std::istream& IStream, 
 				    ILUNPreconditioner<TSparseSpaceType, TDenseSpaceType>& rThis)
   {
   }
  ...
  inline std::ostream& operator << (std::ostream& OStream, 
 				    const ILUNPreconditioner<TSparseSpaceType, TDenseSpaceType>& rThis)
   {
