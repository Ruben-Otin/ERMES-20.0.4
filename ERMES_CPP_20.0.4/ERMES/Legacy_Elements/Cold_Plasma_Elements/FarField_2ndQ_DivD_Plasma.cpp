...
{  	
    ...
    void FarField_2ndQ_DivD_Plasma::jwK_OnGaussPoints( Vector<std::complex<double> >& jwKgp, Matrix<double>& N )
    {
        ...
        { 
            ...
            if (abs(S) > 0.0) K_per2 = mu * eo * ( R * L / S );
            else              K_per2 = mu * eo;
           
            // Square root of K_per2
    ...
    void FarField_2ndQ_DivD_Plasma::GetStiffMatrix( Matrix<std::complex<double> >& StiffMatrix ) 
    {
        ...
        Vector<std::complex<double> > invTr( numGaussPoints );

        for ( gp=0; gp<numGaussPoints; gp++ ) 
        {
            std::complex<double> Traza = ( TEgp[gp][0][0] + TEgp[gp][1][1] + TEgp[gp][2][2] ) / 3.0;

            if ( abs(Traza) > 0.0 ) invTr[gp] = 1.0 / ( Traza * std::conj( Traza ) );
            else                    invTr[gp] = 1.0;
        }
        ...
        for ( i=0; i<numNodes; i++ )
        {
            for ( j=0; j<numNodes; j++ )
            {
                ...
                for ( gp=0; gp<numGaussPoints; gp++ ) 
                {
                    ...
                    int_neNi_rct_neNj += W[gp] * ( jwK[gp] * invTr[gp] ) * ( std::conj(nTEi) *  N[i][gp] ) * ( nTEj * N[j][gp]) ;
                }
