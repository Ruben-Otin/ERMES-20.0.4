...
#include "../ERMES/VolumeElement_2ndQ_FullWave.h"
#include "../ERMES/JSource_2ndQ_FullWave.h"
...
namespace Kratos
{
	...
	void Modeler::GenerateContactPairs( Vector<int>& CtCNodesId )
	{
		...
		if ( mod_epc_First3 > mod_epc_Last3 )
		{
			...
			for ( int i=0; i<3; i++ ) OneSideNodesId[i] = CtCNodesId[i];
			PushHONodesOnSurface( OneSideNodesId, sR_NodesId );

			for ( int i=0; i<3; i++ ) OneSideNodesId[i] = CtCNodesId[i+3];
    ...
    void Modeler::GenerateVolumeElement_ColdPlasma( int* NodesId, unsigned int PropertiesId )
    {
		...
		else if ( mElementOrder == 2 ) pElement = Element::Pointer( new VolumeElement_2ndQ_ColdPlasma( nodes, properties ) );
        ...
        ContactHO_TKT_ColdPlasma( HONodesId, EleIdVector, EleStiffMatrix );
    ...
    void Modeler::GenerateVolumeElement_FullWave( int* NodesId, unsigned int PropertiesId )
    {
		...
		else if ( mElementOrder == 2 ) pElement = Element::Pointer( new VolumeElement_2ndQ_FullWave( nodes, properties ) );
    ...
    void Modeler::GenerateSourceElement( int* NodesId, unsigned int PropertiesId )
    {
	    ...
	    else if ( mElementOrder == 2 ) pElement = Element::Pointer( new JSource_2ndQ_FullWave( nodes, properties ) );
        ...
        if ( mColdPlasmaMode ) ContactHO_TR_ColdPlasma( HONodesId, EleIdVector, EleResVector );
        ...
        if ( mColdPlasmaMode ) ColdPlasma_TR( HONodesId, EleResVector );
    ...
    void Modeler::GenerateFarFieldElement( int* NodesId )
    {
        ...
        //Trans(T)*A*T for contact discontinuities
        if ( mColdPlasmaMode == true ) ContactHO_TKT_ColdPlasma( HONodesId, EleIdVector, EleStiffMatrix );
    ...
    //***********************************************************************************************************
    void Modeler::TangencialCoordinates( std::vector<double>& n, std::vector<double>& t, std::vector<double>& b )
    {
    ...
    //**********************************************************************************************************************************
    void Modeler::AxisymmetricCoordinates( unsigned int NodeId, std::vector<double>& n, std::vector<double>& t, std::vector<double>& b )
    {
		Node::Pointer pNode = mpModel->GetNode(NodeId-1);
    ...
    void Modeler::PBC_RightLeft_TKT( std::vector<int>& HONodesId, Vector<int>& IdVector, Matrix<std::complex<double> >& StiffMatrix )
   {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    void Modeler::ContactHO_TKT_ColdPlasma( std::vector<int>& HONodesId, Vector<int>& IdVector, Matrix<std::complex<double> >& StiffMatrix )
    {
    	if ( mContactPairs.size() == 0 ) return;
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    void Modeler::Get_Contact_Matrix_ColdPlasma( Matrix< std::complex<double> >& ContactMatrix, int RNodesId )
    {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    void Modeler::ContactHO_TR_ColdPlasma( std::vector<int>& HONodesId, Vector<int>& IdVector, Vector<std::complex<double> >& ResVector )
    {
        if ( mContactPairs.size() == 0 ) return;
        ...
        for( it_Back = mPBCBackElements.begin(); it_Back != mPBCBackElements.end(); ++it_Back )
        {
			...
			for( it_eB = elemSetBack.begin(); it_eB != elemSetBack.end() && FrontNodesList.size() > 0 ; ++it_eB )
            {
                ...
                for( it_nFl = FrontNodesList.begin(); it_nFl != FrontNodesList.end(); ++it_nFl )
                {
                    ...
                    if ( sqrt( pow( eX[0] - carX, 2 ) + pow( eY[0] - carY, 2 ) ) > eCross_Section ) 
                    {
                        continue;
                    }
                    ...
                    if ( ( ( natX ) < ( -mPBCTolerance ) ) || 
                         ( ( natY ) < ( -mPBCTolerance ) ) || 
                         ( ( natX + natY ) > ( 1.00 + sqrt(2) * mPBCTolerance ) ) ) 
                    {
                        continue;
                    }
    ...
    int Modeler::PBC_Node_To_Node( double natX, double natY, std::vector<int>& ElementNodeIds )
    {
        ...
        if ( mElementOrder == 0 )
        {
            if ( ( abs(natX - 0.25) < mPBCTolerance ) && ( abs(natY - 0.25) < mPBCTolerance ) ) { return ElementNodeIds[3]; }
            if ( ( abs(natX - 0.50) < mPBCTolerance ) && ( abs(natY - 0.25) < mPBCTolerance ) ) { return ElementNodeIds[4]; }
            if ( ( abs(natX - 0.25) < mPBCTolerance ) && ( abs(natY - 0.50) < mPBCTolerance ) ) { return ElementNodeIds[5]; }
        }
    ...
    void Modeler::SolveProblem()
    {
        ...
        // Updating fixed DOFs
        if ( mColdPlasmaMode ) UpdateCoordPlasmaRLP ();
    ...
    void Modeler::UpdateContactCoord()
    {
        ...
        for (nodes_it=nodes.begin(); nodes_it!= nodes.end(); ++nodes_it)
        {
			if ( mContactPairs.find( (*nodes_it)->Id() ) != mContactPairs.end() )
            {
                ...
                if ( mColdPlasmaMode )
                {
                    Get_Contact_Matrix_ColdPlasma( ContactMatrix, (*nodes_it)->Id() );
                }
                else
                {
                    // Properties
    ...
    void Modeler::Ini_Electrostatic_E_Derivation()
    {
        ...
        mNeigbourElements.resize( nodes.size() ); 
        
        for ( int i=0; i<nodes.size(); i++ ) mNeigbourElements[i] = 0;
	...
	void Modeler::Ini_H_Derivation()
	{
		...
		for ( it=nodes.begin(); it!=nodes.end(); ++it )
        {
		    (*mpModel)(REAL_H, **it) = vCero;
            (*mpModel)(IMAG_H, **it) = vCero;
        }
	}
	...
	void Modeler::Ini_B_Derivation()
	{
		...
		for ( it=nodes.begin(); it!=nodes.end(); ++it )
        {
		    (*mpModel)(REAL_B, **it) = vCero;
            (*mpModel)(IMAG_B, **it) = vCero;
        }
	}
	...
	void Modeler::End_B_Derivation()
	{
        ...
        for ( it=nodes.begin(); it!=nodes.end(); ++it )
        {
            ...
            if ( NumNeighbours > 0 ) 
            {
                B_real /= NumNeighbours;
                B_imag /= NumNeighbours;
            }
                                       ...
                                       ResultsOnGPsType& IMAG_H_OnGP )
	{
        ...
        if ( mElementOrder == 0 ) pElement->SetNormalType( mType_Of_BC_Normal );

        // Element material properties
        double freq    = mProblemFrequency;
        double mo      = (4.00e-7) * 3.1415926535897932384626433832795; 
        ...
        // Magnetic field on nodes
        cVector2Type cH_OnNodes;

        pElement->Calculate_Rotational_OnNodes( cH_OnNodes );

        for ( int n=0; n<cH_OnNodes.size(); n++ ) cH_OnNodes[n] *= inv_jwmu;
		...
		for ( int n=0; n<nodes.size(); n++ )
        {
            ...
            mNeigbourElements[ nodes[n]->Id() - 1 ]++;
        }

        // If smoothing is off -> calculate H on GPs also
        if ( mSmoothing_On == false ) 
        {
            cVector2Type cH_OnGPs;
            
            pElement->Calculate_Rotational_OnGaussPoints( cH_OnGPs, mResultsOnGPs );
            
            for ( int gp=0; gp<cH_OnGPs.size(); gp++ ) cH_OnGPs[gp] *= inv_jwmu;
                                       ...
                                       ResultsOnGPsType& IMAG_B_OnGP )
	{
        ...
        {   
            ...
            Vector<double> rH_OnNodes, iH_OnNodes;
            Vector<double> rB_OnNodes, iB_OnNodes, vAux;
            ...
            for ( int n=0; n<nodes.size(); n++ )
            {
                ...
                rB_OnNodes = rH_OnNodes; rB_OnNodes *=  mu_real;
                vAux       = iH_OnNodes; vAux       *= -mu_imag;
                rB_OnNodes+= vAux;

                iB_OnNodes = iH_OnNodes; iB_OnNodes *= mu_real;
                vAux       = rH_OnNodes; vAux       *= mu_imag;
                iB_OnNodes+= vAux;
        ...
        else
        {
            ...
            std::complex<double> inv_jw( 0.00, -1.00 / mProblemFrequency ); 
            
            cVector2Type cB_OnGPs;
            
            pElement->Calculate_Rotational_OnGaussPoints( cB_OnGPs, mResultsOnGPs );
               
            for ( int gp=0; gp<cB_OnGPs.size(); gp++ ) cB_OnGPs[gp] *= inv_jw;
    ...
    void Modeler::Calculate_E_Element_Electrostatic( int* NodesId, unsigned int PropertiesId, ResultsOnGPsType& ResultsOnGPs )
    {
        ...
        if ( mSmoothing_On == true )
        {
            ...
            for( node_it = nodes.begin(); node_it != nodes.end(); ++node_it )
            {
                ...
                mNeigbourElements[ (*node_it)->Id() - 1 ]++;
	...
	void Modeler::Calculate_J_Induced_OnNodes( int* NodesId, unsigned int PropertiesId )
	{
             ...
             ( mur_real == 1.0 ) && ( mur_imag == 0.0 )  )
        {
		    ...
		    Vector<std::complex<double> > cJv(3), cEv(3);
            
		    Vector<double> rJv(3), iJv(3);
	...
	void Modeler::Calculate_J_Induced_OnGaussPoints( int* NodesId, unsigned int PropertiesId, cVector2Type& cJ_eddy_OnGP )
    {
        ...
        // Induced currents on IHL material 
        Properties::Pointer properties = mpModel->GetProperties( PropertiesId );
        ...
        // Induced currents on cold plasma
        double epr_real = (*properties)( REAL_ELECTRIC_PERMITTIVITY );
        double epr_imag = (*properties)( IMAG_ELECTRIC_PERMITTIVITY );
        double mur_real = (*properties)( REAL_MAGNETIC_PERMEABILITY );
        double mur_imag = (*properties)( IMAG_MAGNETIC_PERMEABILITY );
             ...
             ( mur_real == 1.0 ) && ( mur_imag == 0.0 )  )
        {
            ...
            pElement->Get_InnerGiD_GaussPoints( cX, cY, cZ, mResultsOnGPs );

            Matrix<double> N; 
            
            Lagrange3D_Ni_1st( N, cX, cY, cZ );

            Vector< Matrix< std::complex<double> > > TCn( N.RowsNumber() );

            for ( int n=0; n<N.RowsNumber(); n++ ) 
            {
                mpColdPlasma->Get_ConductivityTensor_InNode( TCn[n], nodes[n], mProblemFrequency );
            }

            Matrix< std::complex<double> > TCgp;

            for ( int gp=0; gp<numGaussPoints; gp++ ) 
            {
                TCgp.Resize( 3, 3, std::complex<double>( 0.00, 0.00 ) );

                for ( int n=0; n<N.RowsNumber(); n++ )
                {
                    TCgp[0][0] += N[n][gp] * TCn[n][0][0]; 
                    TCgp[0][1] += N[n][gp] * TCn[n][0][1];
                    TCgp[0][2] += N[n][gp] * TCn[n][0][2];

                    TCgp[1][0] += N[n][gp] * TCn[n][1][0];
                    TCgp[1][1] += N[n][gp] * TCn[n][1][1];
                    TCgp[1][2] += N[n][gp] * TCn[n][1][2];

                    TCgp[2][0] += N[n][gp] * TCn[n][2][0];
                    TCgp[2][1] += N[n][gp] * TCn[n][2][1];
                    TCgp[2][2] += N[n][gp] * TCn[n][2][2];
                }
                
                cJ_eddy_OnGP[gp][0] = TCgp[0][0]*cE[gp][0] + TCgp[0][1]*cE[gp][1] + TCgp[0][2]*cE[gp][2];
                cJ_eddy_OnGP[gp][1] = TCgp[1][0]*cE[gp][0] + TCgp[1][1]*cE[gp][1] + TCgp[1][2]*cE[gp][2];
                cJ_eddy_OnGP[gp][2] = TCgp[2][0]*cE[gp][0] + TCgp[2][1]*cE[gp][1] + TCgp[2][2]*cE[gp][2];
            }
        }
    }
	...
	//**********************************************************************************************************************
    //* - Calculate J imposed on Gauss points.
    //**********************************************************************************************************************
	void Modeler::Calculate_J_Imposed_OnGaussPoints( int* NodesId, unsigned int PropertiesId, cVector2Type& cJ_imps_OnGP )
    {
        ...
        // Imposed current on vertice nodes
        ...
        // Check if this element is inside a source volume (imposed or imported)
        bool Element_Is_Inside_Source = false;
         
        // Check if this element is inside an imposed source volume
        ...
        // Check if this element is inside an imported source volume
        if ( ( tmp_ist_rJ.find( NodesId[0] ) != tmp_ist_rJ.end() ) &&
        ...
        // If element is not inside a source -> finish
        if ( Element_Is_Inside_Source == false ) return;

        // If this element is inside a source then interpolate nodal values to Gauss points
        std::vector<Node::Pointer> nodes;
    ...
    void Modeler::CalculateNodal( int variableKey )
    {
        ...
        {
            Vector<double> E_total(3), E_real, E_imag;
        ...
        {
            Vector<double> B_total(3), B_real, B_imag;
        ...
        {
            Vector<double> H_total(3), H_real, H_imag;
        ...
        {
            Vector<double> J_total(3), J_real, J_imag;
        ...
        else if ( variableKey == E_PARALLEL_T.getKey() )
        {
            ...
            Vector<double> Bext, b(3);

            double Bnorm, EparDotb;
            double time = mpModel->CurrentTime();
            double freq = mProblemFrequency;
        ...
        else if ( variableKey == POYNTING_VECTOR.getKey() )
        {
            ...
            Vector< std::complex<double> > cEv(3), cHv(3);
        ...
        else if ( variableKey == LORENTZ_FORCE.getKey() )
        {
            ...
            Vector< std::complex<double> > cJv(3), cBv(3);
