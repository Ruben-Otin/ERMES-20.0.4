...
namespace Kratos
{
    ...
    void Kernel::SetProblemType( const String& ProblemType )
    {
	    ...
	    if (ProblemType == "Build")
        {
            ...
            // Set objects needed for building bubble elements
            //Set_Bubble_Building_Objects();
            ...
            // Clear the objects used when building bubble elements
            //Clear_Bubble_Building_Objects();
    ...
    // - Set objects needed for building bubble elements
    //*******************************************************************************************************************
    void Kernel::Set_Bubble_Building_Objects()
    {
        //if ( mpModeler->ElementOrder() != 0 ) return;
        
        mBuild = false;

        mSetConnectivities = true;
        
        String setConn_file_name = mBaseFileName;
        setConn_file_name << "-1.dat";
        GidInput inputDataFile(setConn_file_name);
        inputDataFile.Read(this);
        
        mSetConnectivities  = false;

        //mApplyJumpCondition = true;
        //
        //String applyJumpCond_file_name = mBaseFileName;
        //applyJumpCond_file_name << "-1.dat";
        //GidInput inputJumpCondFile(applyJumpCond_file_name);
        //inputJumpCondFile.Read(this);
        //
        //mApplyJumpCondition = false;

        //mpModeler->Set_Nodal_Connectivities();
        //mpModeler->Set_Bubble_Derivatives  ();
    ...
    // - Clear the objects used when building bubble elements
    //*******************************************************************************************************************
    void Kernel::Clear_Bubble_Building_Objects()
    {
        if ( mpModeler->ElementOrder() != 0 ) return;
        
        mpModeler->Clear_Bubble_Derivatives  ();
        mpModeler->Clear_Nodal_Connectivities();
    }
