...
#include "FarField_3sb_ColdPlasma.h"
...
{  	
    ...
    //********************************************************************************************************************************	
    void FarField_3sb_ColdPlasma::GetEquationIdVector( Vector<int>& EquationId )
    {
    ...
    //* - Area of the element
    //********************************************************************************************************************************	
    double FarField_3sb_ColdPlasma::Calculate_Area() 
    {
    ...
    //* - Exterior normal 
    //********************************************************************************************************************************	
    void FarField_3sb_ColdPlasma::Calculate_ExtNormal( Vector<double>& extN )
    {
        double v1[ 3 ], v2[ 3 ];
    ...
    //********************************************************************************************************************************
    String FarField_3sb_ColdPlasma::Get_Wave_Type()
    {
        String sWaveType;
    ...
    //********************************************************************************************************************************
    void FarField_3sb_ColdPlasma::PermittivityTensor_OnGaussPoints( Vector< Matrix< std::complex<double> > >& TEgp, Matrix<double>& N )
    {
        // Permittivity tensor on main nodes
        Vector< Matrix< std::complex<double> > > TEn( 3 );
        ...
        { 
            mpColdPlasma->Get_PermittivityTensor_OnNode( TEn[ i ], mNodes[ i ], mFreq );
        }
    ...
    //********************************************************************************************************************************
    void FarField_3sb_ColdPlasma::jwK_OnGaussPoints( Vector< std::complex<double> >& jwKgp, Matrix<double>& N )
    {
        // Wave vectors on main nodes
        Vector< std::complex<double> > jwKn( 3 );
    ...
    //********************************************************************************************************************************
    void FarField_3sb_ColdPlasma::Get_invTau_InGaussPoints( Vector< std::complex<double> >& invTau, Vector< Matrix< std::complex<double> > >& TEgp )
    {
	...
	//* - Stiffness matrix
	//********************************************************************************************************************************
	void FarField_3sb_ColdPlasma::GetStiffnessMatrix( Matrix<std::complex<double> >& StiffMatrix )
	{
    ...
    //********************************************************************************************************************************	
    void FarField_3sb_ColdPlasma::Get_Non_Regularized_Matrix( Matrix< std::complex<double> >& StiffMatrix ) 
    {
		// Constants
        double pi = 3.14159265358979;
        double mu = pi * 4.0e-7;
        ...
        for( int i=0; i<mNumNodes; i++ )
        {
            for( int j=0; j<mNumNodes; j++ )
            {
                ...
                StiffMatrix[ i               ][ j               ] = invMu * intg_nxNi_jwK_nxNj_XX; 
                StiffMatrix[ i +   mNumNodes ][ j +   mNumNodes ] = invMu * intg_nxNi_jwK_nxNj_YY; 
                StiffMatrix[ i + 2*mNumNodes ][ j + 2*mNumNodes ] = invMu * intg_nxNi_jwK_nxNj_ZZ; 

                StiffMatrix[ i               ][ j +   mNumNodes ] = invMu * intg_nxNi_jwK_nxNj_XY; 
                StiffMatrix[ i               ][ j + 2*mNumNodes ] = invMu * intg_nxNi_jwK_nxNj_XZ; 
                StiffMatrix[ i +   mNumNodes ][ j + 2*mNumNodes ] = invMu * intg_nxNi_jwK_nxNj_YZ; 

                StiffMatrix[ i +   mNumNodes ][ j               ] = invMu * intg_nxNi_jwK_nxNj_XY; 
                StiffMatrix[ i + 2*mNumNodes ][ j               ] = invMu * intg_nxNi_jwK_nxNj_XZ; 
                StiffMatrix[ i + 2*mNumNodes ][ j +   mNumNodes ] = invMu * intg_nxNi_jwK_nxNj_YZ; 
            }
        }
    }

    //********************************************************************************************************************************
    //* - Stiffness matrix ( Ef formulation )
    //********************************************************************************************************************************	
    void FarField_3sb_ColdPlasma::Get_Stiffness_Matrix_Ef( Matrix< std::complex<double> >& StiffMatrix ) 
    {
		// Constants
        double pi = 3.14159265358979;
        double mu = pi * 4.0e-7;
        ...
        for( int i=0; i<mNumNodes; i++ )
        {
            for( int j=0; j<mNumNodes; j++ )
            {
                ...
                for( int gp=0; gp<numGaussPoints; gp++ ) 
                {
                    ...
                    intg_neNi_rct_neNj_XX += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_X * eNTE_X * N[ i ][ gp ] * N[ j ][ gp ];
                    intg_neNi_rct_neNj_YY += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_Y * eNTE_Y * N[ i ][ gp ] * N[ j ][ gp ];
                    intg_neNi_rct_neNj_ZZ += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_Z * eNTE_Z * N[ i ][ gp ] * N[ j ][ gp ];
                                                                                   
                    intg_neNi_rct_neNj_XY += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_X * eNTE_Y * N[ i ][ gp ] * N[ j ][ gp ];
                    intg_neNi_rct_neNj_XZ += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_X * eNTE_Z * N[ i ][ gp ] * N[ j ][ gp ];
                    intg_neNi_rct_neNj_YZ += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_Y * eNTE_Z * N[ i ][ gp ] * N[ j ][ gp ];
                                                                                                                 
                    intg_neNi_rct_neNj_YX += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_Y * eNTE_X * N[ i ][ gp ] * N[ j ][ gp ];
                    intg_neNi_rct_neNj_ZX += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_Z * eNTE_X * N[ i ][ gp ] * N[ j ][ gp ];
                    intg_neNi_rct_neNj_ZY += W[ gp ] * jwK[ gp ] * invTau[ gp ] * eNTE_Z * eNTE_Y * N[ i ][ gp ] * N[ j ][ gp ];
                }
    ...
    //********************************************************************************************************************************
    //* - Stiffness matrix ( AV formulation )
    //********************************************************************************************************************************	
    void FarField_3sb_ColdPlasma::Get_Stiffness_Matrix_AV( Matrix< std::complex<double> >& StiffMatrix ) 
    {
        // Constants
        double pi = 3.14159265358979;
        double mu = pi * 4.0e-7;
        ...
        for( int i=0; i<mNumNodes; i++ )
        {
            for( int j=0; j<mNumNodes; j++ )
            {
                ...
                for( int gp=0; gp<numGaussPoints; gp++ ) 
                {
                    intg_Ni_jwK_Nj += W[ gp ] * N[ i ][ gp ] * jwK[ gp ] * N[ j ][ gp ];
                }
