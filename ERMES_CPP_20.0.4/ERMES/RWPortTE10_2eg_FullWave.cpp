...
#include "RWPortTE10_2eg_FullWave.h"
...
{  	
    ...
    //*******************************************************************************************************	
    void RWPortTE10_2eg_FullWave::GetEquationIdVector( Vector<int>& EquationId )
    {
    ...
    //* - Area of the element
    //*******************************************************************************************************
    double RWPortTE10_2eg_FullWave::Calculate_Area() 
	{
    ...
    //*******************************************************************************************************
    std::complex<double> RWPortTE10_2eg_FullWave::Calculate_BetaMuCoef() 
	{
    ...
    //*******************************************************************************************************
    std::complex<double> RWPortTE10_2eg_FullWave::Calculate_N10() 
	{
    ...
    //* - Waveguide width
    //*******************************************************************************************************
	double RWPortTE10_2eg_FullWave::Calculate_Width()
	{
		// Corners coordinates
    ...
    //* - Waveguide height
    //*******************************************************************************************************
	double RWPortTE10_2eg_FullWave::Calculate_Height()
	{
		// Corners coordinates
		Vector<double> cNF = (*mProperties)( COMPLEX_NEUMANN_FLOW );
		Vector<double> SSC = (*mProperties)( SINUSOIDAL_SURFACE_CURRENT );
    ...
    //* - b vector ( vertical ) 
    //*******************************************************************************************************
	void RWPortTE10_2eg_FullWave::Calculate_b( double* b ) 
	{
		Vector<double> cNF = (*mProperties)( COMPLEX_NEUMANN_FLOW );
		Vector<double> SSC = (*mProperties)( SINUSOIDAL_SURFACE_CURRENT );
    ...
    //* - t vector ( horizontal ) 
    //*******************************************************************************************************
	void RWPortTE10_2eg_FullWave::Calculate_t( double* t ) 
	{
		Vector<double> cNF = (*mProperties)( COMPLEX_NEUMANN_FLOW );
		Vector<double> SSC = (*mProperties)( SINUSOIDAL_SURFACE_CURRENT );
    ...
    //*******************************************************************************************************
	void RWPortTE10_2eg_FullWave::Calculate_r0( double* r0 ) 
	{
		Vector<double> cNF = (*mProperties)( COMPLEX_NEUMANN_FLOW );
    ...
    //*******************************************************************************************************
	double RWPortTE10_2eg_FullWave::GetPortNumber()
	{
    ...
    //* - Exterior normal 
    //*******************************************************************************************************	
    void RWPortTE10_2eg_FullWave::Calculate_Exterior_Normal( Vector<double>& Next )
    {
		double v1[ 3 ], v2[ 3 ];
    ...
    //*******************************************************************************************************
    void RWPortTE10_2eg_FullWave::Get_CoPlanar_XY_Axis( Vector<double>& X_axis,  Vector<double>& Y_axis )
	{
    ...
    //*******************************************************************************************************
    void RWPortTE10_2eg_FullWave::Get_Vertices_Surface_Coordinates( Vector<double>& pX,  Vector<double>& pY )
    {
    ...
    //*******************************************************************************************************
    void RWPortTE10_2eg_FullWave::Calculate_DN( double DN[ 2 ][ 3 ] )
    {
    ...
    //*******************************************************************************************************
    void RWPortTE10_2eg_FullWave::Calculate_Edge_Lengths( Vector<double>& eLength )
    {
    ...
    //*******************************************************************************************************
	void RWPortTE10_2eg_FullWave::Whitney2D_Ni_1st( Matrix<double>& Nx, Matrix<double>& Ny, Vector<double>& cX, Vector<double>& cY )
	{
		double L1, L2, L3;
    ...
    //******************************************************************************************************************************************
	void RWPortTE10_2eg_FullWave::Face_Base( int edgeBase[ 2 ], int nodeBase[ 2 ], int nodeID[ 3 ], int edgeID[ 3 ], int oppnID[ 3 ] )
	{
    ...
    //******************************************************************************************************************************************
	void RWPortTE10_2eg_FullWave::Order_Face_Bases( int n[ 2 ], int e[ 2 ] )
	{
		// Local face base IDs 
		int edgeBase[ 2 ];
		int nodeBase[ 2 ];
    ...
    //******************************************************************************************************************************************
	void RWPortTE10_2eg_FullWave::Whitney2D_Ni_2nd( Matrix<double>& Nx, Matrix<double>& Ny, Vector<double>& cX, Vector<double>& cY )
	{
		int numPoints = cX.size();
    ...
    //*******************************************************************************************************
    void RWPortTE10_2eg_FullWave::Input_Field_Vector( Vector<double>& Fx, Vector<double>& Fy, Vector<double>& cX, Vector<double>& cY )
	{
        // XY axis of the co-planar coordinate system 
        Vector<double> X_axis, Y_axis;
    ...
    //* - Residual vector 
    //*******************************************************************************************************
    void RWPortTE10_2eg_FullWave::GetResidualVector( Vector< std::complex<double> >& ResidualVector ) 
    {
		...
		for( int i=INI_I; i<END_I; i++ )
        {
            ...
            ResidualVector[ i + POS_I ] = cIntgCte * sIntg_FNi;
        } 
    ...
    //* - Stiffness matrix
    //*******************************************************************************************************	
    void RWPortTE10_2eg_FullWave::GetStiffnessMatrix( Matrix< std::complex<double> >& StiffMatrix ) 
    {
		...
		for( int i=0; i<mNumBases; i++ )
		{
			for( int j=i; j<mNumBases; j++ )
			{
				...
				EdgeBaseMatrix[ i ][ j ] = mcBetaMu * sIntg_NiNj;
			}
		}
    ...
    //*******************************************************************************************************
	std::complex<double> RWPortTE10_2eg_FullWave::GetProjection()
	{
		// Gauss points
		Vector<double> cX, cY, W;
		...
		int NUM_FY = mNumFaces + 2*mNumEdges;
		int POS_IY = mNumVerts;
		int POS_IZ = mNumVerts + 2*mNumEdges - NUM_FY;

		if( mPotentialsOn )
		{
		    for( int e=0; e<NUM_FY; e++ )
		    {
		        cFt[ e ] = (*mProperties)( cAy, *mNodes[ e + POS_IY ] );
		    }

			for( int e=NUM_FY; e<mNumBases; e++ )
		    {
		        cFt[ e ] = (*mProperties)( cAz, *mNodes[ e + POS_IZ ] );
		    }
		}
		else
		{
		    for( int e=0; e<NUM_FY; e++ )
		    {
		        cFt[ e ] = (*mProperties)( cEy, *mNodes[ e + POS_IY ] );
		    }

			for( int e=NUM_FY; e<mNumBases; e++ )
		    {
		        cFt[ e ] = (*mProperties)( cEz, *mNodes[ e + POS_IZ ] );
		    }
		}
    ...
    //*******************************************************************************************************
	std::complex<double> RWPortTE10_2eg_FullWave::GetNormalization()
	{
		// Gauss points
		Vector<double> cX, cY, W;
